...
budget_engine.py
----------------
Core headroom-check logic for the indirect procurement budget control system.

Entry point: ``check_request_headroom``
Returns a ``BudgetCheckResult`` describing whether the request can self-approve,
needs approval routing, or must be blocked.
"""
...
@dataclass
class WindowCheck:
    window_type: str          # transaction | week | month | quarter | year
    limit_type: str           # requester | approver
...
def check_request_headroom(
    ...
    *,
    ...
    """
    Perform a full budget headroom check for a purchase request.

    Checks (in order):
    1. Cost-centre budget headroom for the active period/year version
    2. Company budget cap headroom
    3. Requester's window limits (transaction, week, month, quarter, year)
    ...
    if cc_version:
        ...
        logger.warning(
            f"No active CC budget version for cost_centre={cost_centre_id}, "
            f"year={year_id}, period={period_id} — skipping CC headroom check"
        )

    # ------------------------------------------------------------------
    # 2. Company budget cap
    # ------------------------------------------------------------------
    if year_id:
        cap = (
    ...
    # Filter to limits relevant to the current period
    ...
    if not breaches:
        # No window limit was breached → self-approve
        result.can_self_approve = True
        result.needs_approval = False
    else:
        # At least one window breached → must route for approval
...
def commit_approver_limits(
    ...
    *,
    ...
    """
    Deduct ``amount_minor`` from all active approver window limits for
    ``approver_id`` in ``cost_centre_id``.  Called when a request is approved.
    """
...
def commit_cc_budget(
    ...
    *,
    ...
    # Update company cap committed bucket
    if year_id:
        from provisioning_service.Models import CostCentre
        cc = db.query(CostCentre).filter(CostCentre.cost_centre_id == cost_centre_id).first()
        if cc:
            cap = (
                db.query(CompanyBudgetCap)
                .filter(
                    CompanyBudgetCap.tenant_id == cc.tenant_id,
                    CompanyBudgetCap.year_id == year_id,
                )
                .first()
            )
...
def _resolve_cc_version(db: Session, cost_centre_id, year_id, period_id, today: date):
    ...
    if period_id:
        # Prefer period-specific, then fall back to annual (period_id IS NULL)
...
def _is_limit_in_window(lim, today: date) -> bool:
    """Return True if the limit's window covers today."""
    if lim.window_start and lim.window_end:
        return lim.window_start <= today <= lim.window_end
    return True  # no explicit window → always applicable
