"""
Policy client — provisioning_service.
...
def require_policy(
    ...
    *,
    ...
    Parameters
    ----------
    action                  : Policy action code, e.g. ``"purchase_request.create"``.
    resource_from           : ``"body"`` or ``"none"`` (when resource_loader is None).
    resource_fields         : Subset of body fields to forward as resource context.
    resource_loader         : FastAPI dependency returning a resource context dict.
    pass_on_require_approval: When True, require_approval does not raise; caller decides.
    """
    ...
    # ------------------------------------------------------------------
    # Branch B: extract resource from request body
    # ------------------------------------------------------------------
