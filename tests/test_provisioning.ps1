...
#
# Usage:
#   ./tests/test_provisioning.ps1
#   ./tests/test_provisioning.ps1 -BaseUrl "http://localhost:8080"
#   ./tests/test_provisioning.ps1 -SkipSeed        # Skip internal seed
#   ./tests/test_provisioning.ps1 -SkipOnboarding  # Reuse existing tenant
#   ./tests/test_provisioning.ps1 -TenantId "..." -Token "..."  # Use existing tenant
#   ./tests/test_provisioning.ps1 -NoTranscript    # Don't save results to file
#
# Results are automatically saved to tests/results/provisioning_YYYYMMDD_HHmmss.log
...
# ------------------------------------------------------------------
# Result logging (transcript) — captures all output to a timestamped file
# ------------------------------------------------------------------
if (-not $NoTranscript) {
    ...
    $transcriptFile = Join-Path $ResultDir "provisioning_${timestamp}.log"
    Start-Transcript -Path $transcriptFile -Append | Out-Null
    Write-Host "Test results will be saved to: $transcriptFile" -ForegroundColor DarkGray
}
...
function Write-Fail    {
    $script:FailCount++
    $script:FailedTests += $args
    Write-Host "  [FAIL] $args" -ForegroundColor Red
    if ($StopOnFail) { throw "Test failed: $args" }
}
function Write-Warn    { $script:WarnCount++; Write-Host "  [WARN] $args" -ForegroundColor Yellow }
...
function Invoke-Api {
    ...
    if ($Body) {
        if ($Body -is [string]) {
            ...
            # Auto-inject tenant_id for provisioning routes (not catalog/internal)
            if ($script:TenantId -and $Body -is [hashtable] -and -not $Body.ContainsKey('tenant_id') -and $Path -match '^/(provisioning|onboarding|approved-ranges|financial-calendars|budgets|user-budgets|approval-policies|budget-change-requests|subscriptions|payments|authentication)/') {
                $Body['tenant_id'] = $script:TenantId
            }
    ...
    try {
        ...
        try { $responseBody = $resp.Content | ConvertFrom-Json } catch { $responseBody = $resp.Content }
    }
    catch {
        $respEx = $_.Exception.Response
        if ($respEx) {
            $statusCode = [int]$respEx.StatusCode
            $stream = $respEx.GetResponseStream()
            $reader = New-Object System.IO.StreamReader($stream)
            $content = $reader.ReadToEnd()
            $reader.Close()
            try { $responseBody = $content | ConvertFrom-Json } catch { $responseBody = $content }
        }
        else {
            Write-Fail "$desc - connection error: $_"
            return $null
        }
    }
...
function Invoke-DataIntel {
    ...
    elseif ($AllowNotFound -and ($statusCode -eq 404 -or $statusCode -eq 503)) {
        Write-Warn "$desc - not found yet (worker may still be processing)  ($statusCode)"
    }
    else {
        Write-Fail "$desc - expected $ExpectedStatus, got $statusCode"
    }
...
if (-not $SkipSeed) {
    ...
    foreach ($planCode in @("core_01", "pro_01", "basic_01")) {
        foreach ($featCode in $features.code) {
            Invoke-Api -Method PUT -Path "/internal/plans/$planCode/features/$featCode" -Description "Map $featCode â†- $planCode"
        }
    }
...
if (-not $SkipOnboarding) {
    ...
    # Extract admin user ID from token via whoami
    $who = Invoke-Api -Path "/authentication/whoami" -Description "GET /authentication/whoami (get admin user ID)"
    if ($who.Body -and $who.Body.user_id) {
        $state.AdminUserId = $who.Body.user_id
        Write-Info "Admin User ID (from token): $($state.AdminUserId)"
    } else {
        Write-Warn "Could not extract user_id from token -- org_unit/cost_centre assignments may fail"
    }
}
...
Invoke-Api -Method PUT -Path "/provisioning/tenants/$tid" `
    -Body @{ tenant_id = $tid; name = "Test Corporation Ltd"; type = "retailer"; phone = "+447700900002"; active = $true } `
...
$r = Invoke-Api -Method POST -Path "/provisioning/roles" -Body @{
    code        = "procurement_manager"
    description = "Can create purchase requests and manage budgets"
} -Description "POST /provisioning/roles"
$state.RoleId = $r.Body.role_id
Invoke-Api -Path "/provisioning/roles" -Description "GET /provisioning/roles"

Invoke-Api -Method POST -Path "/provisioning/roles/map-permission?role_code=procurement_manager&permission_code=catalog.products.view" `
...
Invoke-Api -Method POST -Path "/provisioning/users/$($state.UserId)/cost-centres?cost_centre_id=$($state.CostCentreId)&allocated_budget_minor=10000000" `
...
Invoke-Api -Method POST -Path "/approved-ranges/$($state.ApprovedRangeId)/categories$(q)" `
    -Body @{ category_ids = @($state.CategoryId) } `
    -Description "Add categories to approved range"
...
Invoke-Api -Path "/approved-ranges/$($state.ApprovedRangeId)/categories$(q)" -Description "GET categories in range"
...
# ------------------------------------------------------------------
# 21. Budget Change Requests
# ------------------------------------------------------------------
Write-Step "21. Budget Change Requests"
Invoke-Api -Method POST -Path "/budget-change-requests/top-up?tenant_id=$tid&cost_centre_id=$($state.CostCentreId)&from_version_id=$($state.VersionId)&to_version_id=$($state.VersionId)&amount_minor=500000&justification=Emergency%20maintenance%20supplies%20needed" `
...
# ------------------------------------------------------------------
# 23. Data Intelligence -- Graph & Vector Verification
# ------------------------------------------------------------------
if (-not $SkipDataIntelVerify) {
    Write-Step "23. Data Intelligence -- Graph & Vector Verification"

    Write-Info "23.1 Waiting for outbox worker to consume provisioning events..."
    ...
    Write-Info "23.2 Check tenant topology in Neo4j graph"
    Invoke-DataIntel -Path "/graph/tenant/$tid/topology" `
        -Description "DI GET /graph/tenant/{tenant_id}/topology" `
        -AllowNotFound $true

    if ($state.StoreId) {
        Write-Info "23.3 Check store products in Neo4j graph"
    ...
    if ($state.ProductId) {
        Write-Info "23.4 Check product-store mappings in Neo4j graph"
    ...
    if ($state.UserId) {
        Write-Info "23.5 Check user context + hierarchy in Neo4j graph"
        Invoke-DataIntel -Path "/graph/user-context/$($state.UserId)?tenant_id=$tid" `
    ...
    if ($state.UserId) {
        Write-Info "23.6 Check approved products for user (graph + approved ranges)"
        Invoke-DataIntel -Path "/graph/approved-products/$($state.UserId)?tenant_id=$tid" `
            -Description "DI GET /graph/approved-products/{user_id}" `
            -AllowNotFound $true
    }

    if ($state.OrgUnitId) {
        Write-Info "23.7 Check approved products for org unit"
        Invoke-DataIntel -Path "/graph/approved-products/org-unit/$($state.OrgUnitId)?tenant_id=$tid" `
...
Write-Host "                   TEST RESULTS                             " -ForegroundColor $color
Write-Host "============================================================" -ForegroundColor $color
Write-Host ""
...
Write-Host "    ./tests/test_provisioning.ps1 -SkipSeed -SkipOnboarding -TenantId '$($state.TenantId)' -Token 'TOKEN'"
Write-Host "    ./tests/test_provisioning.ps1 -SkipSeed -SkipOnboarding -TenantId '$($state.TenantId)' -Token 'TOKEN' -SkipDataIntelVerify"
Write-Host ""
...
if (-not $NoTranscript) {
    ...
    Write-Host "Results saved to: $transcriptFile" -ForegroundColor DarkGray
}
