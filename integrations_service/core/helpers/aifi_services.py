...
def build_product_payload(product, store_prices: List[Dict] = None) -> Dict:
    ...
    thumb = getattr(product, "thumbnail", None) or "https://example.com/placeholder.png"
    payload = {
...
async def upsert_product(product, existing_by_external: Dict[str, Dict], store_prices: List[Dict] = None) -> Dict:
    ...
    try:
        body = resp.json()
        remote_id = body.get("id") or body.get("product", {}).get("id")
    except Exception:
        remote_id = None
    return {
        "externalId": ext,
...
async def upsert_customer(user, existing_by_external: Dict[str, Dict]) -> Dict:
    ...
    try:
        body = resp.json()
        remote_id = body.get("id") or body.get("customer", {}).get("id")
    except Exception:
        remote_id = None
    return {
        "externalId": ext,
