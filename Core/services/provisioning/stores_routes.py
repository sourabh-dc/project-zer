...
async def create_store(req: StoreRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        site = db.query(Site).filter(Site.site_id == uuid.UUID(req.site_id)).first()
        if not site:
        ...
        logger.info(f"✅ Created store: {store.store_id} ({store.name}) for tenant: {site.tenant_id}")
        return {
...
async def list_stores(
    ...
    q = db.query(Store).join(Site, Store.site_id == Site.site_id).filter(Site.tenant_id == ctx.tenant_id)
