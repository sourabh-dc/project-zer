...
class TenantRequest(BaseModel):
    ...
    admin_first_name: str = Field(min_length=1, max_length=150, description="Admin first name")
    admin_last_name: str = Field(min_length=1, max_length=150, description="Admin last name")
    password: str = Field(min_length=8, max_length=128, description="Admin password (min 8 chars)")
    plan_code: Optional[str] = Field(default=None, description="Subscription plan code (optional)")
    billing_cycle: Optional[str] = Field(default=None, description="Billing cycle: monthly or yearly")
...
class StoreRequest(BaseModel):
    ...
    store_type: Optional[str] = Field(None, description="Store type")
...
class RoleRequest(BaseModel):
    ...
    name: Optional[str] = Field(None, max_length=200, description="Role display name")
...
class CostCentreRequest(BaseModel):
    ...
    tenant_id: str = Field(description="Tenant ID")
    created_by: str = Field(description="User ID who is creating the cost centre")
...
class UserBudgetRequest(BaseModel):
    ...
    allocated_budget_minor: int = Field(ge=0, description="Budget in minor units")
    currency_code: str = Field(default="GBP", max_length=3)
    recurring_budget_minor: int = Field(default=0, ge=0, description="Recurring budget in minor units")
    recurring_period: str = Field(default="none", description="Recurring period: none, monthly, quarterly, yearly")
    spent_minor: int = Field(default=0, ge=0, description="Already spent amount (defaults to 0)")
...
class OrgUnitRequest(BaseModel):
    ...
    type: Optional[str] = Field(None, min_length=1, max_length=50, description="Type: directorate, business_unit, department, team, etc.")
    tenant_id: str = Field(description="Tenant ID")
    parent_org_unit_id: Optional[str] = Field(None, description="Parent org unit ID (optional)")
    cost_centre_id: Optional[str] = Field(None, description="Linked cost centre ID (optional)")


class OrgUnitAssignmentRequest(BaseModel):
    """User to org unit assignment request"""
    assigned_by: Optional[str] = Field(None, description="User ID assigning (optional if taken from context)")
    role_id: Optional[str] = Field(None, description="Role ID to associate with the assignment (required if role is enforced)")
...
class TenantSubscriptionRequest(BaseModel):
    ...
    tenant_id: str = Field(description="Tenant ID")
    plan_selected: str = Field(description="Plan selected")
    payment_method: Optional[str] = Field(default="card", description="Payment method")
    billing_cycle: Optional[str] = Field(default="monthly", description="Billing cycle: monthly, quarterly, yearly")
    created_by: Optional[str] = Field(None, description="User ID creating the subscription (optional; derived from context if available)")
