...
class Site(Base):
    """Site model - physical locations governed by a single tenant"""
    __tablename__ = "sites"
...
class RolePermission(Base):
    ...
    role_permission_id = Column(SQLUUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    role_id = Column(SQLUUID(as_uuid=True), ForeignKey("roles.role_id", ondelete="CASCADE"), nullable=False, index=True)
...
class RoleScope(Base):
    ...
    role_scope_id = Column(SQLUUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    role_id = Column(SQLUUID(as_uuid=True), ForeignKey("roles.role_id", ondelete="CASCADE"), nullable=False, index=True)
    resource_type = Column(String(50), nullable=False, index=True)  # tenant, site, store, user
...
class OrgUnit(Base):
    ...
    tenant_id = Column(SQLUUID(as_uuid=True), ForeignKey("tenants.tenant_id", ondelete="CASCADE"), nullable=False, index=True)
    type = Column(String(50), nullable=True, index=True)  # directorate, business_unit, cost_centre, etc.
    name = Column(String(255), nullable=False)
...
class OrgUnitCostCentre(Base):
    """Mapping between org units and cost centres"""
    __tablename__ = "org_unit_cost_centres"
    ...
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    __table_args__ = (
        Index('ix_org_unit_cost_centre_unique', 'org_unit_id', 'cost_centre_id', unique=True),
    )
...
class UserManager(Base):
    ...
    id = Column(SQLUUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id = Column(SQLUUID(as_uuid=True), ForeignKey("users.user_id", ondelete="CASCADE"), nullable=False, index=True)
    manager_user_id = Column(SQLUUID(as_uuid=True), ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True, index=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class PlanCatalog(Base):
    """Plan catalog (pricing-agnostic definitions)"""
...
class PlanPriceCatalog(Base):
    """Plan price catalog (per plan_code)"""
...
class TenantSubscription(Base):
    ...
    tenant_subscription_id = Column(SQLUUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    ...
    plan_selected = Column(String(50), ForeignKey("plan_catalog.code"), nullable=False)
