...
This is a governance-critical handler. It manages:
  (:ApprovedRange)-[:INCLUDES_CATEGORY]->(:Category)  ← PRIMARY governance path
  (:OrgUnit)-[:GOVERNED_BY]->(:ApprovedRange)
  (:ApprovedRange {is_universal: true})  → applies to all org units

Products become approved implicitly through their category membership.
The approved universe traversal is:
  OrgUnit -[:GOVERNED_BY]-> ApprovedRange -[:INCLUDES_CATEGORY]-> Category <-[:IN_CATEGORY]- Product

Product and Category nodes are created/updated by their own dedicated handlers
(product_handler.py, category_handler.py). This handler only manages the
relationship edges that drive governance.
"""
...
def handle(event: dict):
    ...
    if etype == "approved_range.created":
        ...
        _soft_delete(aid)
    elif etype == "approved_range.org_units_added":
        _map_org_units(aid, payload)
    elif etype == "approved_range.org_unit_removed":
        _unmap_org_unit(aid, payload)
    elif etype == "approved_range.categories_added":
        _add_categories(aid, payload)
    elif etype == "approved_range.category_removed":
        _remove_category(aid, payload)
...
def _add_categories(range_id: str, payload: dict):
    """Add categories to an approved range (PRIMARY governance path).

    Creates (:ApprovedRange)-[:INCLUDES_CATEGORY]->(:Category) edges.
    All products in those categories + subcategories automatically become
    approved for org units governed by this range.
    """
    ...
    with get_session() as session:
        for cid in category_ids:
            ...
            session.run(
                ...
                WITH c
                MATCH (ar:ApprovedRange {approved_range_id: $rid})
                MERGE (ar)-[r:INCLUDES_CATEGORY]->(c)
                SET r.include_subcategories = $include_sub
                """,
                cid=cid_str,
                rid=range_id,
                include_sub=include_subcategories,
            )
    logger.info(f"Graph: {len(category_ids)} categories added to range {range_id}")


def _remove_category(range_id: str, payload: dict):
    ...
    with get_session() as session:
        session.run(
            """
            MATCH (ar:ApprovedRange {approved_range_id: $rid})-[r:INCLUDES_CATEGORY]->(c:Category {category_id: $cid})
            DELETE r
            """,
            rid=range_id,
            cid=str(category_id),
        )
    logger.info(f"Graph: Category {category_id} removed from range {range_id}")
