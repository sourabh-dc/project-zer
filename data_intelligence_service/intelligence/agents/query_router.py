...
async def _vector_search(query_text: str, tenant_id: str, user_id: Optional[str]) -> list:
    """Call the local vector search logic."""
    try:
        from data_intelligence_service.vector.embeddings import embed_text
        from data_intelligence_service.vector.pg_vector import similarity_search

        query_embedding = embed_text(query_text)
        results = similarity_search(
            tenant_id=tenant_id,
            query_embedding=query_embedding,
            approved_product_ids=["__all__"], # bypass governance for intelligence routing
            top_k=20,
        )
        return results
    except Exception as exc:
